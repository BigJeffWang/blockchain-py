...
class TokenNodeConfModel(BaseModel):
    ...
    script_unit = Column(String(16), nullable=False, server_default="0", comment="eos专用 脚本类型不同 调用节点编号不同")
    ...
    @staticmethod
    def get_node_url(coin_id, mold="server", node_env=None, pattern=None, script_unit=None):
        filters = {}
        if not node_env:
            node_env = get_env()
        ...
        if script_unit:
            filters["script_unit"] = str(script_unit)
        conn = None
    ...
    def check_eos(url, port, api_key=None, pattern=None):
        try:
            ...
            if pattern == "fast":
                ...
                return {"latest_block": latest_block, "conn": ec}, None
    ...
    @classmethod
    def get_eos_node_script(cls, pattern=None, script_unit=None):
        return cls.get_node_url(_COIN_ID_EOS, mold="script", pattern=pattern, script_unit=script_unit)
...
class EosConnect(object):
    ...
    def http_get_latest_block(self):
        ...
        if err_no == 0:
            ...
            raise Exception("eos node response error")
    ...
    def http_get_account_related_trx_info(self, page=None, account="luckyparkchn"):
        """
...
if __name__ == "__main__":
    ...
    # res = ecc.http_get_account_related_trx_info(page=2)
    # print(res)

    # print(ecc.http_get_block_detail(36641100))
    # print(ecc)
    ecc = TokenNodeConfModel.get_eos_node_script(script_unit=_ONE_S)
    block_detail = ecc.http_get_block_detail("42840470")
    print(block_detail)