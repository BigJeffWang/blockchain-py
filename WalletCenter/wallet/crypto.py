...
def ripemd160(s):
    ...
    ripemd.update(binascii.unhexlify(s))
...
def sha3_bytes(seed):
    return sha3_256_digest(to_string(seed))
...
def checksum_encode(addr_str):
    ...
    o = ''
    v = big_endian_to_int(sha3_bytes(addr.hex()))
...
class PrivateKey(PrivateKeyBase):
    ...
    def to_b58check_btc(self, testnet=False):  # diy by WangYe
        """ Generates a Base58Check encoding of this private key.
        ...
        """
        version = self.TESTNET_VERSION if testnet else self.MAINNET_VERSION
        version_suffix = self.TESTNET_VERSION if testnet else self.MAINNET_VERSION_SUFFIX
        return base58.b58encode_check(bytes([version]) + bytes(self) + bytes([version_suffix]))
...
class PublicKey(PublicKeyBase):
    ...
    def __init__(self, x, y):
        ...
        self.public_key = None  # diy by WangYe

    def get_public_key_eth(self):  # diy by WangYe
        remainder = int(str(self.point.y)[-1]) % 2
        if remainder:
            prefix = "03"
        else:
            prefix = "02"
        self.public_key = prefix + "%x" % self.point.x
        return "0x" + self.public_key

    def get_public_key_btc(self):  # diy by WangYe
        # remainder = int(str(self.point.y)[-1]) % 2
        # if remainder:
        #     prefix = "03"
        # else:
        #     prefix = "02"
        # self.public_key = prefix + "%x" % self.point.x
        self.public_key = "0" + str((self.point.y & 0x1) + 0x02) + "%x" % self.point.x
        return self.public_key

    def get_address_btc(self):  # diy by WangYe
        # pkbin = binascii.unhexlify(self.public_key)
        # addressbin = ripemd160(binascii.hexlify(hashlib.sha256(pkbin).digest()))
        address = base58.b58encode_check(bytes([self.MAINNET_VERSION]) + self.ripe_compressed)
        return address
...
class HDKey(object):
    ...
    """
    @staticmethod
    def from_b58check_btc(private_key):  # diy by WangYe
        private_key_bytes = base58.b58decode_check(private_key)
        return int.from_bytes(private_key_bytes[1:-1], 'big')

    @staticmethod
    def get_public_key_from_ext(ext_key):
        return PublicKey.from_point(bitcoin_curve.public_key(ext_key))
